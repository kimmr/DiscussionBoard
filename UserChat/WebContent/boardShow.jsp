...
<!-- If the user has been successfully logged in, the userID will be displayed. -->
	<%
		...
		if(userID == null) {
			...
			session.setAttribute("messageContent", "Log in");
		...
		if(boardID == null || boardID.equals("")){
			session.setAttribute("messageType", "Error Message");
			session.setAttribute("messageContent", "Select a post");
			response.sendRedirect("index.jsp");
			return;
		}
	...
	When the title is clicked, it will link to index.jsp-->
		...
		<div class="collpase navbar-collapse" id="bs-example-navbar-collapse-1">
			<ul class="nav navbar-nav">
				...
				<li class="active"><a href="boardView.jsp">Board</a></li>
			</ul>
			<%
				if(userID == null) {
			%>
	...
	<div class="container">
		<table class="table table-bordered table-hover" style="text-align: center; border: 1px solid #dddddd">
			<thead>
				...
				<tr>
					<td style="background-color: #fafafa; color: #000000; width:80px;"><h5>By</h5></td>
				...
				<tr>
					<td style="vertical-align: middle; min-height: 150px; background-color: #fafafa; color: #000000; width:80px;"><h5>Content</h5></td>
					<td colspan="3" style="text-align: lef;"><h5><%= board.getBoardContent() %></h5>
				</tr>
				<tr>
			...
			<tbody>
				<tr>
					<td colspan="5" style="text-align: right;">
						...
						%>
						<a href="boardUpdate.jsp?boardID=<%= board.getBoardID() %>" class="btn btn-success">Edit</a>
						<%
							} 
						%>
						
						<a href="boardView.jsp" class="btn btn-primary" type="submit">Back</a>
						<a href="boardReply.jsp?boardID=<%= board.getBoardID() %>" class="btn btn-info">Reply</a>
						<%
							if(userID.equals(board.getUserID()) || userID.equals("admin")) {
								
						%>
							<a href="boardDelete?boardID=<%= board.getBoardID() %>" class="btn btn-danger" onclick="return confirm('Delete?');">Delete</a>
						<% 
							} 
						%>
					</td>
